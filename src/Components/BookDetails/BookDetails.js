...
const BookDetails = () => {
    ...
    useEffect(() => {
        fetch(
            `${process.env.REACT_APP_SERVER}/bookDetails/${searchParams.get(
                "id"
            )}?user=${user?.email}`
        )
            .then((res) => res.json())
    ...
    useEffect(() => {
        if (deleteData && selectedToDelete !== "") {
            axiosSecure
                .delete(`/deleteBook?id=${selectedToDelete}`)
                .then((res) => res.data)
                .then((data) => {
    ...
    const handleAddWish = (item) => {
        axiosSecure
            .post(`/add-to-wish?user=${user?.email}`, { ...item })
            .then((res) => res.data)
            .then((data) => {
    ...
    const handleRemoveWish = (item) => {
        axiosSecure
            .post(`/delete-wish?user=${user?.email}`, { ...item })
            .then((res) => res.data)
            .then((data) => {
                if (data.deletedCount >= 1) {
                    let temp = { ...bookData };
    ...
    const handleAddToCart = () => {
        ...
        axiosSecure
            .post("/addToCart", { ...data, quantity: 1, email: user?.email })
            .then((res) => res.data)
            .then((data) => {
                if (data.acknowledged) {
                    const temp = { ...bookData };
    ...
    const handleRemoveFromCart = () => {
        axiosSecure
            .post(`/removeFromCart?user=${user?.email}`, {
                bookId: bookData?._id,
            })
            .then((res) => res.data)
            .then((data) => {
            ...
            <div className="detailsClass">
                ...
                <div className="ms-3">
                    ...
                    {bookData?.isPremium && (
                        <h6>
                            Available:{" "}
                            {bookData?.quantities === "1"
                                ? bookData?.quantities + " piece"
                                : bookData?.quantities + " pieces"}
                        </h6>
                    )}
                    ...
                    {user?.role === "admin" && (
                        <div className="mt-1 d-flex">
                            ...
                            <div
                                className="bg-light mx-3 p-1"
                                style={{ cursor: "pointer" }}
                                title="Trash"
                                data-bs-target="#ConfirmModal"
                                data-bs-toggle="modal"
                                onClick={() =>
                                    setSelectedToDelete(bookData?._id)
                                }
                            >
                    ...
                    {parseInt(bookData?.quantities) > 0 ?
                        (!bookData?.isPremium ? (
                            <a
                                href={bookData?.pdf}
                        ...
                        )): <p className="fw-bold text-success d-inline">Out of Stock</p>}
                    {user?.email &&
                        (bookData?.wishlist ? (
                            <button
                                className="btn btn-outline-danger ms-2"
                                onClick={() => handleRemoveWish(bookData)}
                            >
                            ...
                            <button
                                className="btn btn-outline-danger ms-2"
                                onClick={() => handleAddWish(bookData)}
                            >
                                {" "}
                                <i className="bi bi-heart"></i> Add to wish list
                            </button>
                        ))}
                </div>
            </div>
